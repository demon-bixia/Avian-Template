    ...
    <div class="flex w-full m-auto items-center justify-center">
        <div class="relative min-w-full py-1">
            ...
            <div @mouseup="event => moveThumb(event.offsetX)" ref="slider"
                ...
                <!--blue fill-->
                <div ref="fill" class="absolute h-2 w-0 rounded-full bg-indigo-300 transition-all duration-100"
                    :style="{'width': props.initialValue?.toString() || '100%'}"> </div>

                <!--dragable thumb-->
                <div ref="thumb" @mousedown.prevent="handleDrag"
                    class="absolute top-[-1px] h-4 w-4 -ml-2 flex items-center justify-center border rounded-full cursor-pointer active:scale-110 focus:scale-110 bg-indigo-300  border-indigo-300 hover:bg-indigo-400 foucs:bg-indigo-400 active:bg-indigo-400 transition-all duration-100"
