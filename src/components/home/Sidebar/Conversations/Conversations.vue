...
import NoConversation from "../../../reusables/empty/NoConversation.vue";
...
const filteredConversations: Ref<ConversationType[] | undefined> = ref(
  chat.conversations
);
...
watch([searchText, openArchive], () => {
  if (openArchive.value) {
    filteredConversations.value = chat.archivedConversations?.filter(
      (conversation) =>
    ...
    filteredConversations.value = chat.conversations?.filter((conversation) =>
      getName(conversation)
        ?.toLowerCase()
        .includes(searchText.value.toLowerCase())
    );
...
onMounted(() => {
  let conversation = (chat.archivedConversations as ConversationType[]).find(
...
<template>
  <div>
    ...
    >
      ...
      <div v-else>
        <ArchivedButton
          v-if="(chat.archivedConversations as ConversationType[]).length > 0"
          :open="openArchive"
