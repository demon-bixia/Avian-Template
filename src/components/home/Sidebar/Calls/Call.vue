...
const props = defineProps<{
    call: Call,
    active?: boolean,
    openInfoModal?: (call: Call) => void,
    openVoiceCallModall?: () => void,
    endCall?: () => void,
}>();
...
<template>
    <div>
        <component :is="props.active ? 'div': 'button'" :aria-label="'voice call with'" @click="handleOpenInfoModal"
            ...
            <div class="w-full flex flex-col">
                <div class="w-full">
                    ...
                    <div class="flex items-start">
                        ...
                        <!--end Call-->
                        <button v-if="props.active" @click="props.endCall"
                        ...
                        <div v-else class="mr-2">
                            <PhoneXMarkIcon v-if="props.call.status === 'missed'"
                                class="w-[14px] h-[14px] text-red-300 dark:text-red-400" />
                            <PhoneArrowUpRightIcon v-else-if="props.call.status === 'sent'"
                                class="w-[14px] h-[14px] text-green-300 dark:text-green-400" />
                            <PhoneArrowDownLeftIcon v-else-if="props.call.status === 'received'"
                                class="w-[14px] h-[14px] text-green-300 dark:text-green-400" />
                        </div>
                    </div>
                </div>
