...
<template>
    <div v-if="index === 0" :style="{ backgroundImage: `url(${(member as Contact).avatar})`}"
        class="rounded-full bg-cover bg-center" :class="props.large ? ['w-[100px]', 'h-[100px]'] : ['w-7', 'h-7']">
    </div>

    <div v-else-if="props.membersLength === 2 && index === 1"
        :style="{ backgroundImage: `url(${(member as Contact).avatar})`}"
