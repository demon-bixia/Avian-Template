...
<template>
  <div class="w-full">
    ...
    <div
      class="h-[80px] px-5 flex items-center"
    ...
    >
      ...
      <div class="grow mr-5" v-if="!recording">
        <textarea
          ...
          class="resize-none py-[30px] w-full mr-5 outline-none text-sm text-black opacity-60 dark:text-white dark:opacity-70 font-normal leading-4 tracking-[0.16px] placeholder:text-black placeholder:opacity-50 dark:placeholder:text-white dark:placeholder:opacity-70 bg-transparent"
          :class="
            chat.settings['dark-mode'] ? ['scrollbar-dark'] : ['scrollbar']
          "
        ></textarea>
      </div>
      ...
      <!--send message button-->
      <button
        v-if="content && (content as string).length > 0"
