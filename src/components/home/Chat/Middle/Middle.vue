...
const isFollowUp = (index: number, previousIndex: number): boolean => {
  if (previousIndex < 0) {
    ...
    let previousSender =
      props.activeConversation?.messages[previousIndex].sender.id;
...
const getReplyToMessage = (message: MessageType) => {
  if (message.replyTo) {
    let replyToMessage = (
      props.activeConversation as Conversation
...
const handlePinMessage = (messageId: number) => {
  let activeConversationIndex = getConversationIndex(
    (props.activeConversation as Conversation).id
  );
...
<template>
  ...
  >
    <div
      ...
      v-for="(message, index) in (props.activeConversation as Conversation).messages"
      :key="index"
    >
      ...
      <MessageBubble
        ...
        :select-message-to-reply-to="props.selectMessageToReplyTo"
        :reply-to-message="getReplyToMessage(message)"
        :handle-pin-message="handlePinMessage"
      />
    </div>
  </div>
</template>
