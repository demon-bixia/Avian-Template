...
import NoChatSelected from "../../reusables/empty/NoChatSelected.vue";
...
const activeConversation = computed(() => {
  let activeConversation = chat.conversations?.find(
    (conversation) => conversation.id === chat.activeConversationId
  );

  if (activeConversation) return activeConversation;
  else
    return chat.archivedConversations?.find(
      (conversation) => conversation.id === chat.activeConversationId
    );
});
...
const activeChatComponent = computed(() => {
  if (chat.status === "loading" || chat.delayLoading) return loading3;
  else if (chat.activeConversationId) return SelectedChat;
