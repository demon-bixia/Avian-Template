...
<template>
  <div>
    <div class="mb-6 px-5 flex justify-between items-center">
      ...
      >
        <span
          v-if="(props.conversation as Conversation).type === 'couple' || props.contact"
          >Contact</span
        >
        <span v-else-if="(props.conversation as Conversation).type === 'group'"
          >Group</span
        >
        <span
          v-else-if="(props.conversation as Conversation).type === 'boradcast'"
          >Broadcast</span
        >
        Info
      </Typography>
        ...
        <div class="w-full flex justify-between">
          <div>
            ...
            <Typography variant="body-2" class="font-extralight text-start">
              <!--last seen-->
              <span
                v-if="(props.conversation as Conversation).type === 'couple' || contact"
              >
              ...
              <span
                v-else-if="['group', 'boradcast'].includes((props.conversation as Conversation).type)"
              >
                {{ (props.conversation as Conversation).contacts.length }}
                Contacts
              </span>
            </Typography>
          </div>
          ...
          <IconButton
            v-if="['group', 'boradcast'].includes((props.conversation as Conversation).type)"
            class="group w-7 h-7"
          >
            <PencilIcon
    ...
    <div class="w-full py-5 border-t border-gray-100 dark:border-gray-700">
      <!--(contact) email-->
      <div
        v-if="(conversation as Conversation).type === 'couple' || props.contact"
      ...
      <!--(group) members-->
      <div
        v-if="['group', 'boradcast'].includes((props.conversation as Conversation).type) && !props.contact"
