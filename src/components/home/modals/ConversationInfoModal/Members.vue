...
const dropdownMenuStates: Ref<boolean[] | undefined> = ref(props.conversation.contacts?.map(() => false));
...
const handleToggleDropdown = (event: Event, contactIndex: number) => {
    if (contactContainer) {
        ...
        let containerBottom = (contactContainer.value as HTMLElement).getBoundingClientRect().bottom;

        if (buttonBottom >= containerBottom - 50) { dropdownMenuPosition.value = ['bottom-6', 'right-0'] }
        else { dropdownMenuPosition.value = ['top-6', 'right-0'] }
    }

    dropdownMenuStates.value = props.conversation.contacts?.map((value, index) => {
...
const handleClickOutside = (event: Event) => {
    ...
    if (target.parentElement && !target.classList.contains('open-menu')
                ...
                <template v-slot:menu
                    ...
                    <div>
                        ...
                        <IconButton @click="event => handleToggleDropdown(event, index)" class="open-menu w-6 h-6">
                            <EllipsisVerticalIcon class="open-menu h-5 w-5 text-black opacity-60 dark:text-white"
                                tabindex="0" />
                        </IconButton>
