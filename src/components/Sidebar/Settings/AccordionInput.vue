...
<template>
    ...
    <div v-if="props.setting.type === 'file'">
        <DropFileUpload @file-selected="event => props.handleQueuedValueChange(event, index)" :id="props.setting.title"
            :label="props.setting.title" :description="props.setting.description" :type="props.setting.type"
            :value="(props.value as FileValue).value" :setting-id="props.setting.id" :group-id="props.groupId" />
    </div>
</template>