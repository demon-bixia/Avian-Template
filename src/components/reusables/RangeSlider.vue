...
const moveThumb = (location: number) => {
  ...
  modelValue.value =
    (location / (slider.value as HTMLElement).clientWidth) * 100;
...
const handleDrag = (event: MouseEvent) => {
  shiftX.value =
    event.clientX - (thumb.value as HTMLElement).getBoundingClientRect().left;
...
const handleMouseMove = (event: MouseEvent) => {
  let newLeft =
    event.clientX -
    shiftX.value -
    (slider.value as HTMLElement).getBoundingClientRect().left;
...
<template>
  <div class="flex w-full m-auto items-center justify-center">
    <div class="relative min-w-full py-1">
      <!--track-->
      <div
        @mouseup="(event) => moveThumb(event.offsetX)"
        ref="slider"
      ...
      >
        ...
        <div
          ref="fill"
          class="absolute h-2 w-0 rounded-full transition-all duration-100"
          :class="props.self ? ['bg-white'] : ['bg-indigo-300']"
          :style="{ width: props.initialValue?.toString() || '100%' }"
        ></div>
        ...
        >
          <div class="relative -mt-2 w-1">
            <div
              class="absolute z-40 bottom-100 left-0 min-w-full mb-2 opacity-100"
